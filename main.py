...
class IPO:
    def __init__(self, info_dict, subscription_sheet, workday_sheet, history=None):
        ...
        self.purchase_ID = info_dict["purchase_ID"]
...
class Allotment():
    def __init__(self, info_dict):
        ...
        self.quote = round(info_dict["quote"],2)
...
class excel_writer:
        ...
        for ipo in ipos:
            ...
            for allotment in ipo.allotment_subjects:
                if not allotment.valid:
                    continue
                # print(ipo.ID)
                # print(allotment.name)
                # print(allotment.valid_subscription_amount)
    ...
    def today_purchase(self, data):
        ...
        for ipo in ipos:
            ...
            write(calendar, [0], [ipo.ID + "   " + str(int(ipo.purchase_ID)).zfill(6) + "  " + ipo.IPO_name], [title_format])
            write(calendar, range(0,len(titles)), titles, [table_cell_format for i in range(len(titles))])
...
if __name__ == '__main__':
    ...
    today = datetime(2021,12,29) ### for testing
    ...
    # receiver = ["wujn@huianfund.cn","jinhuilin990919@163.com"]

    sender = "893125905@qq.com"
    authorization_code = "zhcozzyvxxvzbeei"
